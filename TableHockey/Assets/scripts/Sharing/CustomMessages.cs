...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class CustomMessages : Singleton<CustomMessages>
    {
        ...
        public void SendCollisionVectInfo(Vector3 vector,Vector3 position) //衝突の時の力が加わる方向
        {
            // If we are connected to a session, broadcast our head info
            if (serverConnection != null && serverConnection.IsConnected())
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.CollisionVect);
        ...
        public void SendMalletTransform(Vector3 position, Quaternion rotation)
        {
            // If we are connected to a session, broadcast our head info
            if (serverConnection != null && serverConnection.IsConnected())
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.MalletTransform);
        ...
        public void SendJudgeInfo()
        {
            ...
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.Judge);
        ...
        public Color ReadColor(NetworkInMessage msg)
        {
            return new Color(msg.ReadFloat(), msg.ReadFloat(), msg.ReadFloat());
        }

        public Vector3 ReadCollisionVect(NetworkInMessage msg)
        {
            return new Vector3(msg.ReadFloat(), msg.ReadFloat(), msg.ReadFloat());
        }

        public Vector3 ReadStartVect(NetworkInMessage msg)
        {
            return new Vector3(msg.ReadFloat(), msg.ReadFloat(), msg.ReadFloat());
        }
        #endregion
